...
void test_communcation(TimerHandle_t timer)
{
    ...
    struct timeval lv_time;

    cJSON_AddStringToObject(data, "test", "test");
    gettimeofday(&lv_time, NULL);
    ESP_LOGI("main.c","[test_communcation] send test: %llu:%07lu", lv_time.tv_sec, lv_time.tv_usec);
#if IS_ROOT
...
cJSON *receive_test(cJSON *payload, uint32_t seq)
{
    ...
    ESP_LOGI("main.c", "[receive_test] receved packet %llu:%06lu", lv_t.tv_sec, lv_t.tv_usec);
#if !IS_ROOT
    ...
    esp_mesh_lite_try_sending_msg("answer","answer_ack",0,payload,esp_mesh_lite_send_msg_to_root);
...
cJSON * receive_answer(cJSON *payload, uint32_t seq)
{
    struct timeval lv_t;
    gettimeofday(&lv_t, NULL);
    ESP_LOGI("main.c", "[receive_answer] receved packet %llu:%06lu", lv_t.tv_sec, lv_t.tv_usec);
    return NULL;
}
cJSON * answer_ack(cJSON * payload, uint32_t seq){
    return NULL;
}
...
esp_mesh_lite_msg_action_t action[] = {
    ...
    {"answer", "answer_ack", receive_answer},
    {"answer_ack", NULL, send_test_ack},
...
extern "C" void app_main()
{
    ...
    while (1)
    {
        ...
        if (pin_pressed_edge)
        {
            ...
            Serial.printf("Button pressed: %llu:%06lu\n", l_time.tv_sec, l_time.tv_usec);
            //l_time.tv_sec = 0;
            //l_time.tv_usec = 0;
            //settimeofday(&l_time, NULL);
