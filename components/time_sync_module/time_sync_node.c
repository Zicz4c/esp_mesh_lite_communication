...
cJSON * handle_first_sync_time(cJSON * payload, uint32_t seq){
    ...
    {   
        cJSON * mac_part = cJSON_CreateNumber(own_mac.addr[i]);
...
cJSON * handle_root_sync_time(cJSON * payload, uint32_t seq){
    ...
    if(item_for_mac == NULL){
        //ESP_LOGI(TAG, "own MAC: "MACSTR, MAC2STR(own_mac.addr));
    ...
    ESP_LOGI(TAG, "Delay: %ld.%06ld", d_rn.s, d_rn.us);
    cJSON * data = cJSON_CreateObject();
    gettimeofday(&t_n, NULL);
    ...
    cJSON * data_mac = cJSON_AddArrayToObject(data, JSON_MAC);
    mac_addr_t mac;
    esp_wifi_get_mac(ESP_IF_WIFI_STA, &mac.addr);
    for (size_t i = 0; i < MAC_ADDR_SIZE; i++)
    {   
